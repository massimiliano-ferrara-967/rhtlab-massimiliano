 ...
 "cells": [
  {
   ...
   "metadata": {},
   "source": [
    "Created on 01/12/25, from \"main_multi-energy system_simple_v4\"\\\n",
    "The program is fundamentally unchanged but input data is now more precise, justified with references.\\\n",
    "Among the changes:\n",
    "- The power consumption of the compressors is now evaluated properly\n",
    "- The remuneratoin of FCR is now based on a real profile (fixed remuneration price is left as an option)"
   ]
  },
  {
   "cell_type": "markdown",
  ...
  },
  {
   ...
   "source": [
    ...
    "# - C_hc will multiply the mass flow of hydrogen produced by the EL\n",
    "# - Without 3600 and 1e6, the formula evaluates the power consumption for the compression of a kg/s of H2 in J/kg\n",
    "# - Since we will multiply C_hc by an hydrogen mass flow in kg/h, the division by 3600 is needed\n",
    "# - The result of the multiplication would then be in J/s = W\n",
    "# - Finally, the division by 1e6 brings the result to the MW scale\n",
    "C_hc = (1 / (3600 * Mm * eta_cis)) * Zs * R * T * (ks / (ks - 1)) * ((p_hss / p) ** ((ks - 1) / ks) - 1) / 1e6    # power requirement for the compression of a hydrogen mass unit [MW/kg]\n",
    "\n",
    "\n",
    ...
    "# - The result of the multiplication would then be in J/s = W\n",
    "# - Finally, the division by 1e6 brings the result to the MW scale\n",
    "C_hc = (1 / (3600 * Mm * eta_cis)) * Zs * R * T * (ks / (ks - 1)) * ((p_hss / p) ** ((ks - 1) / ks) - 1) / 1e6    # power requirement for the compression of a hydrogen mass unit [MW/kg] (previously 0.001 for tests) ### NO REFERENCE ###\n",
    "Zs\n"
   ]
  },
  {
   ...
   "source": [
    ...
    "\n",
    "# # (NO) Evaluation of the maximum frequency deviation for which the EL can participate to the FCR service\n",
  ...
  },
  {
   ...
   "source": [
    ...
    "rho_elpos = {'EL': 0.4 * (Pel_rtd_W / 1e6) * 3600}\n",
    "rho_elneg = {'EL': -0.5 * (Pel_rtd_W / 1e6) * 3600}\n",
    "\n",
    ...
    "rho_fcpos = {'FC': 0.07 * 0.5 * 3600}\n",
    "rho_fcneg = {'FC': -0.046 * 0.5 * 3600}\n",
    "\n",
    "\n",
    ...
    "m.C_FCR_capup = pyo.Param(initialize = 20, domain = pyo.NonNegativeReals)    ### NO REFERENCE ###\n",
    "m.C_FCR_capdown = pyo.Param(initialize = 20, domain = pyo.NonNegativeReals)    ### NO REFERENCE ###\n",
    "\n",
  ...
  },
  {
   ...
   "source": [
    ...
    "#                 + (m.Ppv[t] + sum(m.pb_dch[b, t] - m.pb_ch[b, t] for b in m.BESS) \\\n",
    "#                 - sum(m.pel[e, t] for e in m.EL) + sum(m.pfc[f, t] for f in m.FC)) * m.Dt * m.C_ele[t] \\\n",
    "#                 + sum(m.pel_fcr_sym[e, t] * m.C_FCR_capup + m.pel_fcr_sym[e, t] * m.C_FCR_capdown for e in m.EL) \\\n",
    "#                 + sum(m.pfc_fcr_up[f, t] * m.C_FCR_capup + m.pfc_fcr_dwn[f, t] * m.C_FCR_capdown for f in m.FC) for t in m.TIME)\n",
    "# m.Obj = pyo.Objective(rule = tot_revenue, sense = pyo.maximize)       \n",
    "\n",
    "\n",
